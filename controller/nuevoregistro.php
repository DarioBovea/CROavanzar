<?php
    ...
    try {
        ...
        if ($results>0) {
            ...
            if ($stmt->execute()) {
                $message = 'El usuario ha sido creado exitosamente, por favor Inicie Sesion';
            }
        }
    } catch (PDOException $e) {
        $message = 'Lo sentimos, ha ocurrido un error creando al nuevo usuario, vuelva a';
    }
    $stmt->closeCursor();
    $stmt = null;
    $pdo = null;   
    // echo "<script>window.location= '../view/loguin.php' </script>";
